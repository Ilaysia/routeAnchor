...
async def get_tago_nodes(lat: float, lon: float, session: aiohttp.ClientSession) -> list[tuple[str, str]]:
    ...
    params = {
        ...
        "gpsLong": str(lon),
        "_type": "json",
        "numOfRows": "5",  # 🌟 [타임아웃 방지] 10개에서 5개로 줄여서 탐색 속도 극대화
        "pageNo": "1"
    }
            ...
            if isinstance(res, dict):
                for bus_no, times in res.items():
                    if bus_no not in merged_bus_info:
                        ...
                        merged_bus_info[bus_no] = list(dict.fromkeys(all_times))[:2]
    ...
    async with aiohttp.ClientSession() as session: 
        async with session.post(url, headers=headers, json=payload) as response:
            ...
            try:
                ...
                # 🌟 [트래픽 최적화] 가장 좋은 1등, 2등 경로의 버스만 조회하여 Vercel 과부하 방지
                if is_first_segment:
                    for path in itineraries[:2]:
                ...
                if is_first_segment and unique_bus_coords:
                    ...
                    try:
                        # 🌟 [Vercel 절대 방어막] TAGO API가 4.5초 안에 응답 안 하면 가차없이 끊어버립니다. 
                        # 서버 전체가 뻗는 Timeout(500 에러)을 완벽하게 차단합니다.
                        ...
                        pass # 공공데이터 서버가 느리면 에러 없이 안전하게 넘기고 시간표를 띄웁니다.
                ...
                for idx, path in enumerate(itineraries[:10]):
                    ...
                    for leg in path.get("legs", []):
                        ...
                        if mode == "WALK":
                            ...
                            elif mode == "SUBWAY" or mode == "TRAIN":
                                if is_first_segment and idx < 2:
                                    for r_name in route_names:
...
async def process_optimized_route(request: RouteRequest):
    ...
    for i in range(len(all_points) - 1):
        ...
        task = fetch_segments_from_tmap(
            ...
            opt_type=request.optimizationType.value, search_date=request.searchDate, is_first_segment=is_first
        )
